...
# Code Template V 1.2
# URL: https://au2mator.com/disable-computer-account-active-directory-self-service-with-au2mator/
...
function Send-ServiceMail ($HTMLBody, $ServiceName, $Recipient, $RequestID, $RequestStatus) {
    Write-au2matorLog -Type INFO -Text "Function Send-ServiceMail started"
...
try {
    Get-ADComputer -Identity $c_Computer | Disable-ADAccount -Confirm:$false
...
if ($ErrorCount -eq 0) {
    $au2matorReturn = "Computer $c_Computer disabled"
    $AdditionalHTML = "<br>
    Computer " + (Get-ADComputer -identity $c_Computer).DisplayName + " has been disabled
        <br>
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to disable Computer $c_Computer, Error: $Error"
    $Status = "ERROR"
}
